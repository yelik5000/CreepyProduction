...
namespace std {
    template<>
    struct hash<cpe::Model::Vertex> {
        size_t operator()(cpe::Model::Vertex const &vertex) const {
            size_t seed = 0;
            cpe::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
            return seed;
        }
    };
}
